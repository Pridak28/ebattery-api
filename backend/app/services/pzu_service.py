...
class PZUService:
    ...
    def simulate(self, params: PZUSimulationParams) -> PZUSimulationResponse:
        ...
        # Group by day and compute arbitrage. Mastermind R-4: count days
        # rejected for data quality vs days where simulate() returned a
        # zero-trade result (no profitable chronological pair).
        daily_results = []
        days_input = 0
        days_rejected_data = 0
        days_no_profit = 0
        for date_val, day_data in df.groupby(df["date"].dt.date):
            ...
            if result is None:
                days_rejected_data += 1
                continue
            result["date"] = date_val
            daily_results.append(result)
            if (result.get("charge_start_hour") is None) or float(result.get("net_profit", 0.0)) <= 0.0:
                days_no_profit += 1
        ...
        # Bankability decomposition (B1): per-day revenue + cost so the
        # investment service doesn't have to reconstruct from monthly avg
        # prices × cycle-per-day assumption. ``charge_grid_mwh`` /
        # ``discharge_grid_mwh`` are AC-side energies actually paid for /
        # received from the grid on each profitable day; days with no
        # profitable chronological pair contribute 0 to both.
        results_df["daily_gross_revenue_eur"] = (
            results_df["avg_sell_price"] * results_df["discharge_grid_mwh"]
        )
        results_df["daily_energy_cost_eur"] = (
            results_df["avg_buy_price"] * results_df["charge_grid_mwh"]
        )
        ...
        total_gross_revenue = float(results_df["daily_gross_revenue_eur"].sum())
        total_energy_cost = float(results_df["daily_energy_cost_eur"].sum())
        ...
        return PZUSimulationResponse(
            ...
            total_gross_revenue_eur=total_gross_revenue,
            total_energy_cost_eur=total_energy_cost,
            days_input_count=days_input,
            days_simulated_count=len(daily_results),
            days_no_profitable_pair=days_no_profit,
            days_rejected_data_quality=days_rejected_data,
        )
    ...
    def get_typical_day(self) -> PZUTypicalDayResponse:
        """Get typical daily charge/discharge pattern (screening helper).

        Non-chronological — picks cheapest/priciest hour-of-day across the
        entire dataset. Use ``simulate()`` for bankable revenue.
        """
        return self.get_typical_day_pattern()

    def get_typical_day_pattern(
        self,
        start_date: Optional[date] = None,
        end_date: Optional[date] = None,
        num_charge_hours: Optional[int] = None,
        num_discharge_hours: Optional[int] = None,
    ) -> PZUTypicalDayResponse:
        """Date-range + configurable-hours version of ``get_typical_day``.

        Wired by ``GET /api/v1/pzu/typical-day``. Previously the route called
        a method with this exact name that didn't exist, returning HTTP 500.
        """
        df = self._load_price_data()
        ...
        if end_date is not None:
            df = df[df["date"] <= pd.Timestamp(end_date)]
        if df.empty:
            raise ValueError("No PZU data in requested range")

        n_charge = int(num_charge_hours) if num_charge_hours else self.BLOCK_HOURS
        n_discharge = int(num_discharge_hours) if num_discharge_hours else self.BLOCK_HOURS
        ...
        charge_hours = [int(h) for h in sorted_hours.head(n_charge).index]
        discharge_hours = [int(h) for h in sorted_hours.tail(n_discharge).index]
        ...
        avg_buy = float(hourly_avg[charge_hours].mean()) if charge_hours else 0.0
        avg_sell = float(hourly_avg[discharge_hours].mean()) if discharge_hours else 0.0
    ...
    def get_monthly_optimal(self) -> PZUMonthlyOptimalResponse:
        ...
        for month, month_df in df.groupby("month"):
            ...
            # Estimate profit for this month using canonical battery defaults
            # (10 MW × BLOCK_HOURS, 88% RTE) instead of the previous hardcoded
            # 15 MW / 30 MWh sizing. Asymmetric form ``(sell × η − buy) × E``
            # is algebraically identical to the symmetric sqrt(η) form when E
            # is grid-side charge energy and num_charge == num_discharge.
            days_in_month = len(month_df["date"].dt.date.unique())
            power_mw = float(settings.DEFAULT_POWER_MW)
            efficiency = float(settings.DEFAULT_RTE_AC_AC)
